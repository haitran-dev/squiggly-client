...
export default class SignaturePad extends SignatureEventTarget {
	...
	public toDataURL(type: 'image/svg+xml', encoderOptions?: ToSVGOptions): string;
	...
	public fromData(pointGroups: PointGroup[], { clear = true }: FromDataOptions = {}): void {
		if (clear) {
			this.clear();
		}
	...
	private _drawDot(point: BasicPoint, options: PointGroupOptions): void {
		const ctx = this._ctx;
		const width =
			options.dotSize > 0 ? options.dotSize : (options.minWidth + options.maxWidth) / 2;
